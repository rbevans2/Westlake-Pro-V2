...
function bindEvents(){
  ...
  byId("documentApplyTax").addEventListener("change",calcDocTotals);byId("documentTaxRate").addEventListener("input",calcDocTotals);byId("documentSearch").addEventListener("input",renderDocuments);byId("documentCustomer").addEventListener("change",fillDocumentCustomer);byId("documentType").addEventListener("change",handleDocumentTypeChange);
...
function clearDocument(){
  ["documentId","documentNotes","documentCustomerName","documentCustomerPhone","documentCustomerEmail","documentCustomerAddress"].forEach(id=>setVal(id,""));
  setVal("documentType","Invoice");setVal("documentStatus","Draft");setVal("documentCustomer","");setVal("documentJob","");
  setVal("documentNumber",nextNumber("Invoice"));setVal("documentDate",today());setVal("documentDue",addDays(15));
  byId("lineItems").innerHTML="";addLine();byId("documentApplyTax").checked=false;setVal("documentTaxRate",state.business?.default_tax_rate||6);calcDocTotals();
}
function nextNumber(type){
  const prefix=type==="Estimate"?"EST":"INV";
  let maxNum=1000;
  state.documents.filter(d=>d.type===type&&d.number).forEach(d=>{
...
function handleDocumentTypeChange(){
  if(!val("documentId"))setVal("documentNumber",nextNumber(val("documentType")));
}
function fillDocumentCustomer(){
  const c=state.customers.find(x=>x.id===val("documentCustomer"));
  setVal("documentCustomerName",c?.name||"");
  setVal("documentCustomerPhone",c?.phone||"");
  setVal("documentCustomerEmail",c?.email||"");
  setVal("documentCustomerAddress",c?.address||"");
}
...
async function saveDocument(){
  const t=calcDocTotals(),c=state.customers.find(x=>x.id===val("documentCustomer")),j=state.jobs.find(x=>x.id===val("documentJob"));
  const type=val("documentType");
  let number=val("documentNumber").trim()||nextNumber(type);
  if(!val("documentId")&&state.documents.some(d=>d.type===type&&String(d.number).toLowerCase()===number.toLowerCase()))number=nextNumber(type);
  const rec={business_id:businessId,type,status:val("documentStatus"),customer_id:val("documentCustomer")||null,
    customer_name:val("documentCustomerName")||c?.name||"",customer_phone:val("documentCustomerPhone")||c?.phone||"",
    customer_email:val("documentCustomerEmail")||c?.email||"",customer_address:val("documentCustomerAddress")||c?.address||"",
    job_id:val("documentJob")||null,job_title:j?.title||"",number,doc_date:val("documentDate"),due_date:val("documentDue"),
    line_items:getLines(),apply_tax:byId("documentApplyTax").checked,tax_rate:Number(val("documentTaxRate")||0),
    subtotal:t.subtotal,tax:t.tax,total:t.total,notes:val("documentNotes")};
  await upsert("documents",val("documentId"),rec);await load("documents");clearDocument();renderAll();
}
function editDocument(id){
  const d=state.documents.find(x=>x.id===id);if(!d)return;
  setVal("documentId",d.id);setVal("documentType",d.type);setVal("documentStatus",d.status);setVal("documentCustomer",d.customer_id||"");setVal("documentJob",d.job_id||"");
  setVal("documentCustomerName",d.customer_name||"");setVal("documentCustomerPhone",d.customer_phone||"");setVal("documentCustomerEmail",d.customer_email||"");setVal("documentCustomerAddress",d.customer_address||"");
  setVal("documentNumber",d.number);setVal("documentDate",d.doc_date);setVal("documentDue",d.due_date);byId("lineItems").innerHTML="";
  (d.line_items||[]).forEach(i=>addLine(i.description,i.amount));byId("documentApplyTax").checked=!!d.apply_tax;setVal("documentTaxRate",d.tax_rate);setVal("documentNotes",d.notes||"");calcDocTotals();showView("documents");
}
