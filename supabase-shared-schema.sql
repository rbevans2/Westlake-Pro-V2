...
-- Westlake v2 extra shared app fields
alter table public.businesses add column if not exists invoice_terms text default 'Payment due within 15 days.';
alter table public.businesses add column if not exists payment_instructions text default 'Please make checks payable to Westlake Tree Experts.';

alter table public.customers add column if not exists property_address text;

alter table public.jobs add column if not exists crew text;
alter table public.jobs add column if not exists estimate_id uuid references public.documents(id) on delete set null;
alter table public.jobs add column if not exists invoice_id uuid references public.documents(id) on delete set null;

alter table public.services add column if not exists category text default 'Tree Service';
alter table public.services add column if not exists taxable boolean default false;

alter table public.documents add column if not exists property_address text;
...
alter table public.documents add column if not exists payment_instructions text;
alter table public.documents add column if not exists signature text;
...
drop policy if exists "payments business access" on public.payments;
create policy "payments business access" on public.payments for all using (public.is_business_member(business_id)) with check (public.is_business_member(business_id));
